...
class VectorChunkMetadata(BaseModel):
    ...
    doc_type: str = Field(DocTypeEnum.API_REFERENCE.value, description="Type of documentation")
    section_header: str = Field(..., description="Heading or endpoint path for the chunk")
    section_level: int = Field(1, ge=1, le=6, description="Markdown header level (# = 1, ## = 2)")
    ...
    start_char_offset: Optional[int] = Field(None, ge=0, description="Start character offset")
    end_char_offset: Optional[int] = Field(None, ge=0, description="End character offset")
    file_name: str = Field(..., description="Source file name (e.g., auth_v2.md)")
    endpoint_path: Optional[str] = Field(None, description="API endpoint path if applicable (e.g., /api/v2/auth)")
    http_method: Optional[str] = Field(None, description="HTTP Method (GET, POST, PUT, DELETE)")
    is_deprecated: bool = Field(False, description="Whether this section contains deprecated API info")
